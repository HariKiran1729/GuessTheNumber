class GuessTheNumberGameGitHub {
    constructor() {
    ...
    // Use GitHub Gists as database
    async saveGameData(data) {
        try {
            console.log('Saving game data to GitHub Gist for room:', this.roomCode);
            
            const gistData = {
                description: `Guess The Number Game - Room ${this.roomCode}`,
                public: false,
                files: {
                    [`game_${this.roomCode}.json`]: {
            ...
            if (this.gistId) {
                ...
                if (response.ok) {
                    ...
                    console.log('Created gist with ID:', this.gistId);
                    
                    // Store the gist ID mapping locally
                    const gistMapping = JSON.parse(localStorage.getItem('gistMapping') || '{}');
                    gistMapping[this.roomCode] = this.gistId;
                    localStorage.setItem('gistMapping', JSON.stringify(gistMapping));
                }
            }
    ...
    async loadGameData(roomCode) {
        try {
            ...
            // Get gist ID from local mapping
            const gistMapping = JSON.parse(localStorage.getItem('gistMapping') || '{}');
            const gistId = gistMapping[roomCode];
            
            if (!gistId) {
                console.log('No gist ID found for room:', roomCode);
                return null;
            }
            ...
            if (response.ok) {
                ...
                const filename = `game_${roomCode}.json`;
    ...
    async joinGame() {
        ...
        if (!gameData) {
            document.getElementById('join-error').textContent = `Room "${roomCode}" not found. Make sure the room code is correct.`;
            return;
        }
        ...
        // Get the gist ID for updates
        const gistMapping = JSON.parse(localStorage.getItem('gistMapping') || '{}');
        this.gistId = gistMapping[roomCode];
